...
def deploy_container():
    ...
    subprocess.run([
        "docker", "buildx", "build", "--platform", "linux/amd64", "--output", "type=docker", "-t", "lineup-optimizer", "."
    ], check=True, shell=True)

    # Tag for ECR
    ecr_repo = f"{AWS_ACCOUNT_ID}.dkr.ecr.us-east-1.amazonaws.com/lineup-optimizer"
    subprocess.run([
        "docker", "tag", "lineup-optimizer:latest", f"{ecr_repo}:latest"
    ], check=True, shell=True)
    ...
    try:
        ...
        response = lambda_client.create_function(
            ...
            PackageType='Image',
            Description='NBA Lineup Optimizer - Generates optimal DraftKings lineup using predictions',
            Timeout=300,
            MemorySize=512,
            Environment={
                'Variables': {}
