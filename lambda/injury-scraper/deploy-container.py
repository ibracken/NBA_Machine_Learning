...
def deploy_container():
    ...
    subprocess.run([
        "docker", "buildx", "build", "--platform", "linux/amd64", "--output", "type=docker", "-t", "injury-scraper", "."
    ], check=True, shell=True)

    # Tag for ECR
    ecr_repo = f"{AWS_ACCOUNT_ID}.dkr.ecr.us-east-1.amazonaws.com/injury-scraper"
    subprocess.run([
        "docker", "tag", "injury-scraper:latest", f"{ecr_repo}:latest"
    ], check=True, shell=True)
    ...
    try:
        ...
        response = lambda_client.create_function(
            ...
            PackageType='Image',
            Description='NBA Injury Scraper Lambda Function (ESPN)',
            Timeout=300,  # 5 minutes should be plenty
            MemorySize=512,  # Less memory needed - just HTTP scraping
            Environment={
                'Variables': {
