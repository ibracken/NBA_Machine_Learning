...
def deploy_container():
    ...
    # Tag for ECR
    ecr_repo = f"{AWS_ACCOUNT_ID}.dkr.ecr.us-east-1.amazonaws.com/daily-predictions"
    subprocess.run([
        "docker", "tag", "daily-predictions:latest", f"{ecr_repo}:latest"
    ], check=True, shell=True)
    ...
    try:
        response = lambda_client.update_function_code(
            FunctionName='daily-predictions',
        ...
        response = lambda_client.create_function(
            ...
            PackageType='Image',
            Description='NBA Daily Predictions - Scrapes projections and generates predictions',
            Timeout=900,
            MemorySize=2048,
            Environment={
                'Variables': {}
