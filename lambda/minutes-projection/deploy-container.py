...
def deploy_container():
    ...
    # Tag for ECR
    ecr_repo = f"{AWS_ACCOUNT_ID}.dkr.ecr.us-east-1.amazonaws.com/minutes-projection"
    subprocess.run([
        "docker", "tag", "minutes-projection:latest", f"{ecr_repo}:latest"
    ], check=True, shell=True)
    ...
    try:
        response = lambda_client.update_function_code(
            FunctionName='minutes-projection',
        ...
        response = lambda_client.create_function(
            ...
            PackageType='Image',
            Description='NBA Minutes Projection - Complex position overlap with TRUE baseline tracking',
            Timeout=900,
            MemorySize=3008,
            Environment={
                'Variables': {}
